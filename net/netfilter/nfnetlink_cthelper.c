...
nfnl_cthelper_to_nlattr(struct sk_buff *skb, const struct nf_conn *ct)
{
	...
	if (help->helper->data_len) {
		    NLA_PUT(skb, CTA_HELP_INFO, help->helper->data_len,
				&help->data);
	}
	return 0;
			 ...
			 struct nf_conntrack_helper *helper)
{
	...
	NLA_PUT_U16(skb, NFCTH_TUPLE_L3PROTONUM,
			htons(helper->tuple.src.l3num));
	NLA_PUT_U8(skb, NFCTH_TUPLE_L4PROTONUM, helper->tuple.dst.protonum);
			...
			struct nf_conntrack_helper *helper)
{
	...
	NLA_PUT_U32(skb, NFCTH_POLICY_SET_NUM, htonl(helper->expect_class_max));
	...
	for (i=0; i<helper->expect_class_max; i++) {
		...
		NLA_PUT_STRING(skb, NFCTH_POLICY_NAME,
				helper->expect_policy[i].name);

		NLA_PUT_U32(skb, NFCTH_POLICY_EXPECT_MAX,
				htonl(helper->expect_policy[i].max_expected));

		NLA_PUT_U32(skb, NFCTH_POLICY_EXPECT_TIMEOUT,
			...
			int event, struct nf_conntrack_helper *helper)
{
	...
	NLA_PUT_STRING(skb, NFCTH_NAME, helper->name);

	NLA_PUT_BE32(skb, NFCTH_QUEUE_NUM, htonl(helper->queue_num));
	...
	NLA_PUT_BE32(skb, NFCTH_PRIV_DATA_LEN, htonl(helper->data_len));
	...
	NLA_PUT_BE32(skb, NFCTH_STATUS, htonl(status));
		  ...
		  const struct nlmsghdr *nlh, const struct nlattr * const tb[])
{
	...
	if (nlh->nlmsg_flags & NLM_F_DUMP) {
		return netlink_dump_start(nfnl, skb, nlh,
					nfnl_cthelper_dump_table, NULL, 0);
	}
